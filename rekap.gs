...
function regenerateRekapPerBulan(bulan, tahun) {
  var ss = getSpreadsheet();
  var sheet = ss.getSheetByName('data_harian');
  var data = sheet.getDataRange().getValues();
  var header = data[0];
  ...
  // Hapus rekap lama untuk bulan ini saja
  var rekapSheet = ss.getSheetByName('rekap_bulanan');
  var rekapData = rekapSheet.getDataRange().getValues();
  var hapus = [];
  for (var r = rekapData.length - 1; r >= 1; r--) {
    if (!rekapData[r][0]) continue;
    var bulanKey = String(rekapData[r][1]);
    var parts = bulanKey.split('-');
    if (parseInt(parts[0]) === bulanInt && parseInt(parts[1]) === tahunInt) {
      hapus.push(r + 1);
    }
  }
  // Hapus dari bawah ke atas biar index gak geser
  for (var h = 0; h < hapus.length; h++) {
    rekapSheet.deleteRow(hapus[h]);
  }

  // Kumpulkan kombinasi unik untuk bulan ini
  var kombinasi = {};
  for (var i = 1; i < data.length; i++) {
    var row = data[i];
    if (!row[0]) continue;
  ...
  Logger.log('Bulan ' + bulan + '/' + tahun + ' selesai! Kombinasi: ' + count);
}
