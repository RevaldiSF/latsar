...
function buatSheetHarian(sheet, dataHari, terminal, tglHariStr, hari, bulan, tahun) {
  var r = 1;
sheet.getRange(1, 1, 200, 20).setFontFamily('Calibri');
  // AKAP
  sheet.getRange(r, 1).setValue('PENCATATAN ARUS AKAP - TERMINAL ' + terminal.toUpperCase());
  sheet.getRange(r, 1, 1, 19).merge().setFontWeight('bold').setHorizontalAlignment('center'); r++;
  ...
  sheet.getRange(r, 1, 1, 18).setValues(hAKAP).setFontWeight('bold').setBackground('#d9d9d9').setHorizontalAlignment('center').setBorder(true,true,true,true,true,true); r++;
  ...
  if (dataHari.AKAP.length === 0) {
    ...
    dataHari.AKAP.forEach(function(d, idx) {
      var rowData = [idx+1, d.jam_datang, d.nomor_bus, d.koperasi, d.nama_po, d.jml_po, d.BK, d.BS, d.BB, d.jml_seat, d.jml_pnp, d.pnp_turun, d.pnp_naik, d.jam_berangkat, d.trayek, d.laik, d.tidak_laik, ''];
      sheet.getRange(r, 1, 1, 18).setValues([rowData]).setBorder(true,true,true,true,true,true).setHorizontalAlignment('center');
      totAKAP.po = Math.max(totAKAP.po, Number(d.jml_po)||0);
  ...
  sheet.getRange(r, 1, 1, 18).setValues([['JUMLAH','','','','',totAKAP.po,totAKAP.BK,totAKAP.BS,totAKAP.BB,totAKAP.seat,totAKAP.pnp,totAKAP.turun,totAKAP.naik,'','',totAKAP.laik,totAKAP.tidak,'']]).setFontWeight('bold').setBackground('#f0f0f0').setBorder(true,true,true,true,true,true).setHorizontalAlignment('center'); r += 3;
  ...
  sheet.getRange(r, 1, 1, 19).setValues(hAKDP).setFontWeight('bold').setBackground('#d9d9d9').setHorizontalAlignment('center').setBorder(true,true,true,true,true,true); r++;
  ...
  if (dataHari.AKDP.length === 0) {
    ...
    dataHari.AKDP.forEach(function(d, idx) {
      var rowData = [idx+1, d.jam_datang, d.nomor_bus, d.koperasi, d.nama_po, d.jml_po, d.BK, d.BS, d.BB, d.MPU, d.jml_seat, d.jml_pnp, d.pnp_turun, d.pnp_naik, d.jam_berangkat, d.trayek, d.laik, d.tidak_laik, ''];
      sheet.getRange(r, 1, 1, 19).setValues([rowData]).setBorder(true,true,true,true,true,true).setHorizontalAlignment('center');
      totAKDP.po = Math.max(totAKDP.po, Number(d.jml_po)||0);
  ...
  sheet.getRange(r, 1, 1, 19).setValues([['JUMLAH','','','','',totAKDP.po,totAKDP.BK,totAKDP.BS,totAKDP.BB,totAKDP.MPU,totAKDP.seat,totAKDP.pnp,totAKDP.turun,totAKDP.naik,'','',totAKDP.laik,totAKDP.tidak,'']]).setFontWeight('bold').setBackground('#f0f0f0').setBorder(true,true,true,true,true,true).setHorizontalAlignment('center');
}
...
function getDataByNomorBus(nomorBus) {
  var ss = getSpreadsheet();
  var sheet = ss.getSheetByName('data_harian');
  var data = sheet.getDataRange().getValues();
  
  var nomorUpper = String(nomorBus).trim().toUpperCase();
  var found = null;
  
  // Cari dari bawah (data terbaru) ke atas
  for (var i = data.length - 1; i >= 1; i--) {
    var row = data[i];
    if (!row[0]) continue;
    var rowNomor = String(row[10]).trim().toUpperCase();
    if (rowNomor === nomorUpper) {
      found = {
        po_id: row[6],
        nama_po: row[7],
        koperasi: row[8],
