...
function login(username, password) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (String(row[1]).trim() === String(username).trim() && 
      ...
      // Tentukan halaman awal berdasarkan role dan terminal
      var homePage = 'dashboard';
      if (role === 'petugas') {
        if (terminal === 'PURBALINGGA') homePage = 'input_purbalingga';
        else if (terminal === 'BANJARNEGARA') homePage = 'input_banjarnegara';
      }
